...
```bash
# Validated: --source builds via Dockerfile/buildpacks, --update-secrets takes BOTH forms comma-separated (/path=SECRET:version mounts volume always-latest, KEY=SECRET:version sets env startup-pinned), --labels alias of --update-labels (per help run deploy). No --dry-run supported. GOTCHA 2026-09-04: --set-secrets and --update-secrets are mutually exclusive (one group) — first CD failed on it; combined form below is the fix. GOTCHA 2026-09-05: wireJournal() reads PROJECT_ID from env, which Cloud Run does NOT set by itself — without --set-env-vars the service boots health-only and /api/vaults 404s (verified live). --set-env-vars is a different flag group, so it combines freely. GOTCHA 2026-09-06: --set-build-env-vars does NOT reach Dockerfile ARG (verified live: deploy green, bundle baked undefined, browser threw Missing Firebase web config).
GOTCHA 2026-09-06 (2): secret VOLUME mounts resolve `:latest` at container start, not live — a new secret version (e.g. maps-api-key v3) needs a rollout (any push) before serving traffic sees it; verified live (fresh 403s after v3 until revision rolled). Firebase web config rides --set-env-vars instead, into the server-rendered /firebase-config.js route; values come from repo Variables VITE_FIREBASE_API_KEY/AUTH_DOMAIN/PROJECT_ID/APP_ID (Firebase console → Project settings → Your apps) — never repo files, the Sonar hook flags key-shaped strings including build output. Maps browser key rides the same flag/shape into /maps-config.js (repo Variable MAPS_BROWSER_KEY, referrer-restricted browser key from console Credentials).
gcloud run deploy personal-gemini-journal --source . --region us-central1 --allow-unauthenticated --update-secrets=/secrets/maps-api-key=maps-api-key:latest,GEMINI_API_KEY=gemini-api-key:latest --set-env-vars=PROJECT_ID=valid-meridian-475214-e3,FIREBASE_WEB_API_KEY=$VITE_FIREBASE_API_KEY,FIREBASE_WEB_AUTH_DOMAIN=$VITE_FIREBASE_AUTH_DOMAIN,FIREBASE_WEB_PROJECT_ID=$VITE_FIREBASE_PROJECT_ID,FIREBASE_WEB_APP_ID=$VITE_FIREBASE_APP_ID,MAPS_BROWSER_KEY=$MAPS_BROWSER_KEY --service-account=$RUN_SA --labels=challenge=codelab,app=grounded-journal,dev-tutorial=cloud-run-ai-challenge --project=$PROJECT_ID --quiet
...
- `2026-09-04T21:00Z` — deploy live at https://personal-gemini-journal-56gpfvk5zq-uc.a.run.app (revision 00001-7nt, Ready, startup probe passed). Gotcha: exact `/healthz` is intercepted upstream of the revision (never logged server-side; case/slash variants all 200) — probe moved to `/api/health` (`736ac39`), verified live after redeploy.
- `2026-09-05T08:00Z` — sign-in slice live (13c0a8e + f026629, CD green): Firebase Google provider enabled, web app registered, run.app domain authorized, web key restricted (referrers + Identity Toolkit/Token Service APIs only) after GitHub secret-scanning flagged the committed public key — restriction is the real boundary, secrecy was never it. Live / serves the sign-in build (verified). Only human step left: click through Google sign-in in Brave to prove the popup → token → vault chain.
- `2026-09-05T08:30Z` — sign-in WORKS end-to-end; first live Reflect returned the designed 429 (AI Studio prepayment depleted — known since the key-staging ping). Chain proven: popup → token → vault → entry → grounding → model call refused on billing. Unblocks on AI Studio billing top-up (human step).
- `2026-09-05T05:00Z` — journal API live: POST /api/vaults/v1/entries → 201 with real Firestore id (first CD after PROJECT_ID fix, `844a47a`). Before the fix the service booted health-only (router unmounted, Express 404s). Probe doc + vault deleted after verification — prod DB clean.
- `2026-09-04T20:25Z` — second CD failed in 36s (IAM, not code): deployer `github-cd` lacks `artifactregistry.repositories.get` on `cloud-run-source-deploy` (us-central1). Needs a human grant bundle (artifactregistry.writer + cloudbuild.builds.editor + secretmanager.viewer); run.admin + serviceAccountUser alone are insufficient for `--source` deploys. No workflow change — rerun after granting.
- `2026-09-04T20:35Z` — pre-created `cloud-run-source-deploy` (docker, us-central1) as owner to end the auto-create ambiguity. Rerun then failed on `storage.buckets.create` for the `run-sources-*` staging bucket: deployer needs `storage.admin` too (standard for `--source` flows). secretmanager.viewer still ungranted — likely next. No code change; rerun after granting.
- `2026-09-06T15:30Z` — frontend stack switched vanilla → React+Vite per user approval (no gcloud run yet — local build only): `app/client/` scaffold (react 19, vite 6, firebase 11, TS strict), `vite build` emits to `app/public` (emptyOutDir; 50 modules, ~370KB JS / ~100KB gzip); Dockerfile gains a client stage (`npm ci --prefix ./app/client` + `npm run build`, final `COPY --from=build /srv/app/public` so the image never depends on host output); `server.test.ts` static assertions resolve hashed Vite assets from the served shell; client `tsc --noEmit` clean, server suite 82/82 green (store tests still need the emulator on :8090); impeccable detector degraded (parser modules absent, regex fallback, no findings — undercount, contrast hand-verified 5.8–15:1). Vite output (`app/public/index.html` + `assets/`) is UNTRACKED build output (gitignored) — build the client before server tests/serving; Firebase web config rides VITE_ env (`app/client/.env` locally, repo Variables in CI/CD, Dockerfile ARG, `--set-build-env-vars` per §4). Next: create the 4 repo Variables (human step), then `gcloud run deploy` (unchanged §4 command shape + build-env-vars) ships the React build on next CD push.
- `2026-09-06T16:00Z` — build-env-vars path DEAD on arrival: deploy green, but the live bundle baked `undefined` for all four VITE_ vars (browser threw Missing Firebase web config) — `--set-build-env-vars` never reaches Dockerfile ARG. Switched to runtime config: Express serves `/firebase-config.js` from `FIREBASE_WEB_*` env (new route + server tests, TDD red→green), client reads `window.__FIREBASE_CONFIG__` with VITE_ `.env` fallback for `vite dev`; Dockerfile ARG/ENV removed; `--set-env-vars` carries the four values from the same repo Variables (created 2026-09-06, verified via `gh variable list`); local runs export shell env (no dotenv — see `.env.example` tail). Vite output stays untracked; `ci.yml` builds the client before server tests.
- `2026-09-06T16:20Z` — runtime-config fix deployed (CD green): live `/firebase-config.js` serves the real web config (verified values match console block), Missing-config error gone. Root cause as suspected: `--set-build-env-vars` never reaches Dockerfile ARG.
- `2026-09-06T16:40Z` — live white-screen (`turns.length` of undefined in history map): legacy vault entries predate the `turns` field. Fixed at the client seam (`normalizeEntry` in `app/client/src/api.ts`, proven against the crash shape via esbuild-bundled execution) + `ErrorBoundary` fallback so no malformed record can white-screen the journal again.
- `2026-09-06T17:20Z` — history map pins slice (user-supplied referrer-restricted browser key, never persisted in repo/chat): `location` kept through Places normalization + `detailsFromCache` (TDD red→green; pre-location cache records stay pin-less, display path never fetches so zero Places quota on views); Express `/maps-config.js` from `MAPS_BROWSER_KEY` env (tested both paths); React `MapPane` (one-time loader, escaped info windows with attribution, legacy/ungrounded-aside meta, degrade-to-list fallback) behind the List/Map toggle; CD `--set-env-vars` carries the key from new repo Variable `MAPS_BROWSER_KEY` (human step, same console-credentials source as the value); suite 88/88, parity clean, secrets scan clean, two-axis review findings fixed (glossary sweep incl. pre-existing comment/test titles, vite maps-config proxy, pin attribution, dedupe helper).
- `2026-09-06T17:40Z` - map follow-ups: empty map view renders a visibly empty field (the auto-bounce made the tab feel dead); explicit per-place refresh (POST places/:placeId/refresh upgrades cached records with coordinates, 404 when never cached; TDD red-green incl. emulator store tests) with a "Resync N places" affordance in both map branches so all-legacy vaults can upgrade from the view; shared placeLookupError + buildRecord helpers; glossary sweep finished (thread/conversation/message/unattached in store comments and test titles). Full suite 125/125 under emulator, parity + scan clean.
- `2026-09-06T18:00Z` - Resync 502 diagnosed server-side, not client: ALL server Places calls (refresh + autocomplete) fail — console key "API key 4" (server key, staged 9/4) allows elevation/embed/maps/static-maps but NOT places.googleapis.com (verified via api-keys metadata; APIs themselves ENABLED). Fix is console-only: add Places API to that key's restrictions (~5 min propagation), no code needed. Client loader switched to the recommended loading=async bootstrap (kills the direct-load console warning). gen_204 ERR_BLOCKED_BY_CLIENT + runtime.lastError noise is the user's adblocker, harmless.
- `2026-09-06T18:20Z` - Legacy-entry 500s (remove-grounding + reflections on pre-turns docs): store boundary now normalizes records (`normalizeEntryRecord` in repository.ts, pure unit tests, applied in getEntry/listEntries/requireOwnedEntry). Upstream Places still 403s at this write — server key value mismatch stands; retry Resync after propagation, else re-stage the current console key string via Secret Manager New version. Full suite 128/128 under emulator, parity clean.
- `2026-09-06T17:00Z` — Gemini key rotated per user request (new value in local `.env`): staged as `gemini-api-key` version [2] via `secrets versions add --data-file=-` (stdin-fed from `.env` by a Temp Node script — exact bytes, no trailing newline, value never logged; v1 retained enabled). GOTCHA: `GEMINI_API_KEY=gemini-api-key:latest` is env startup-pinned, so the live revision keeps v1 until the next rollout — redeploy (next CD push) to take effect. Verified via `secrets versions list` (metadata only).
